...
app.post('/api/transactions/action', (req, res) => {
  ...
  if (action === 'approve') {
    ...
    // For withdrawal: since we did NOT deduct during request (isAdmin was false), nothing to refund here!
    // For deposit: request was rejected, no change to balance.
  }
  ...
  const { passwordHash, ...safeUser } = user;
  res.json({
    message: action === 'approve' ? 'อนุมัติรายการสำเร็จแล้วค่ะ' : 'ปฏิเสธรายการสำเร็จแล้วค่ะ',
    transaction: tx,
    user: { ...safeUser, rawPassword: passwordHash }
  });
});

// 4. Fetch All Users (Admin Only Management View)
app.get('/api/admin/users', (req, res) => {
  // Return all users for management
  usersCache = loadUsers();
  const safeUsers = usersCache.map(({ passwordHash, ...user }) => ({
    ...user,
    rawPassword: passwordHash // expose for high-control admin usage as requested
  }));
  res.json(safeUsers);
});

// 5. Admin Create Member Account Directly
app.post('/api/admin/users/create', (req, res) => {
  const { username, phone, password, country, accountCategory } = req.body;

  if (!username || !phone || !password || !country) {
    return res.status(400).json({ error: 'กรุณากรอกข้อมูลสำคัญ (ชื่อ, เบอร์โทร, รหัสผ่าน, และประเภทบัญชีลูกค้าประจำประเทศ) ให้ครบทุกช่องค่ะ' });
  }

  if (phone.length !== 10 || !/^\d+$/.test(phone)) {
    return res.status(400).json({ error: 'เบอร์โทรศัพท์ต้องเป็นตัวเลข 10 หลักค่ะ' });
  }

  usersCache = loadUsers();
  const duplicate = usersCache.some(u => u.phone === phone);
  if (duplicate) {
    return res.status(409).json({ error: 'เบอร์โทรศัพท์นี้ถูกใช้งานในระบบแล้วค่ะ' });
  }

  const invitationCode = country === 'Laos' ? 'lo4385' : 'th3585';
  const newUser: UserAccount = {
    username,
    phone,
    bankName: 'ธนาคารกสิกรไทย',
  ...
  const success = saveUsers(usersCache);
  if (!success) {
    return res.status(500).json({ error: 'ไม่สามารถสร้างบัญชีสมาชิกระบบหลังบ้านได้' });
  }

  res.status(201).json({ message: 'สมัครสมาชิกสำเร็จเรียบร้อยแล้วค่ะ', user: newUser });
});

// 6. Admin Update Member Data (Admin Control Panel)
app.post('/api/admin/users/update', (req, res) => {
  const { phone, balance, commission, frozen, level, isBlocked, isBanned, role, username, realName, bankName, bankAccount, rawPassword, transactionPassword, accountCategory, country, latestDepositAmount } = req.body;

  if (!phone) {
    return res.status(400).json({ error: 'กรุณาระบุเบอร์โทรศัพท์ผู้ใช้ที่ต้องการแก้ไข' });
  }
  ...
  if (userIdx === -1) {
    return res.status(404).json({ error: 'ไม่พบบัญชีผู้ใช้ที่ต้องการแก้ไข' });
  }

  // Prevent blocking the main admin
  if (phone === '0909090909' && (isBlocked === true || isBanned === true)) {
    return res.status(400).json({ error: 'ไม่สามารถระงับหรือแบนผู้ใช้งานแอดมินระดับสูงสุดของระบบได้ค่ะ' });
  }

  // Update fields
  if (typeof balance === 'number') {
    const oldBalance = usersCache[userIdx].balance || 0;
    const diff = parseFloat((balance - oldBalance).toFixed(2));
    
    if (diff !== 0) {
      usersCache[userIdx].balance = balance;
      
      // Log transaction in transactions history
      const txs = loadTransactions();
      const newTx: Transaction = {
        id: (diff > 0 ? 'TXD-' : 'TXW-') + Date.now() + '-' + Math.floor(1000 + Math.random() * 9000),
        phone,
        username: usersCache[userIdx].username,
        type: diff > 0 ? 'deposit' : 'withdrawal',
        amount: Math.abs(diff),
        status: 'approved',
        note: 'ปรับปรุงยอดเงินโดยแอดมิน (แผงควบคุม)',
        createdAt: new Date().toISOString()
      };
      txs.push(newTx);
      saveTransactions(txs);
  ...
  if (typeof commission === 'number') usersCache[userIdx].commission = commission;
  if (typeof frozen === 'number') usersCache[userIdx].frozen = frozen;
  if (typeof latestDepositAmount === 'number') {
    usersCache[userIdx].latestDepositAmount = latestDepositAmount;
    usersCache[userIdx].level = getLevelByLatestDeposit(latestDepositAmount);
  } else if (typeof level === 'number') {
    usersCache[userIdx].level = level;
  }
  ...
  if (realName) usersCache[userIdx].realName = realName;
  if (bankName) usersCache[userIdx].bankName = bankName;
  if (bankAccount) usersCache[userIdx].bankAccount = bankAccount;
  if (rawPassword) usersCache[userIdx].passwordHash = rawPassword;
  ...
  const success = saveUsers(usersCache);
  if (!success) {
    return res.status(500).json({ error: 'เกิดข้อผิดพลาดในการอัปเดตข้อมูลผู้ใช้ในระบบ' });
  }

  res.json({ message: 'อัปเดตข้อมูลสมาชิกเรียบร้อยแล้วค่ะ', user: usersCache[userIdx] });
});

// 7. Manage Admins (Webmaster Phone 0909090909 Only)
app.post('/api/admin/admins/update', (req, res) => {
  const { actorPhone, targetPhone, newPhone, newPassword, newUsername, action } = req.body;

  if (!actorPhone) {
    return res.status(400).json({ error: 'ไม่พบข้อมูลผู้ดำเนินการ' });
  }

  usersCache = loadUsers();
  const actor = usersCache.find(u => u.phone === actorPhone);
  if (!actor || actor.role !== 'admin') {
    return res.status(403).json({ error: 'ไม่มีสิทธิ์เข้าถึงส่วนงานแอดมินค่ะ' });
  }

  // Webmaster Phone '0909090909' is required to configure admins
  if (actorPhone !== '0909090909') {
    return res.status(403).json({ error: 'เฉพาะบัญชีผู้ดูแลระบบสูงสุด (0909090909) เท่านั้นที่สามารถจัดสรรแอดมินระดับสูงสุดในระบบได้ค่ะ' });
  }
